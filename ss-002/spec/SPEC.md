...
description: The normative Wallet-gateway specification — (SS-002). 
document: stablecoin-stack/ss-002/SPEC.md
spec: FPSF-SS-002
version: 1.0.0
status: Draft
date: 2026-03-10
...
# FPSF-SS-002 — wallet-gateway Interface Specification
...
- the internal protocol between wallet-gateway and broadcast-service (planned: SS-SPEC-005);
- the internal protocol between wallet-gateway and balance-and-history;
- the on-chain Settlement Contract interface (specified in SS-SPEC-001, Sections 10–13); or
...
|---|---|
| SS-SPEC-001 v1.0.0 | The Stablecoin Stack — foundational specification |
...
All terms defined in SS-SPEC-001 Section 2.4 carry the same meaning in this document. Additional terms introduced here:
...
The values in `BrokenDownAmount` correspond directly to the return values of `breakdownTransferAmount` on the Settlement Contract (SS-SPEC-001, Section 12.3): `remaining` maps to `principalAmount` and `fees` maps to `totalFees`.
...
Submits a signed `TransferRequest` payload (SS-SPEC-001, Section 8.2) for processing. This is an asynchronous operation: the gateway acknowledges receipt immediately and subsequently delivers status updates via the notification mechanism defined in Section 10.
...
interface SubmitPaymentPayload {
  requestId: string;
  transferRequest: TransferRequest; // As defined in SS-SPEC-001, Section 8.2.
}
```
...
Submits a signed `BuyAcquiringPackRequest` payload (SS-SPEC-001, Section 8.3) for processing. The operation is asynchronous in the same manner as payment submission.
...
interface SubmitAcquiringPayload {
  requestId: string;
  buyAcquiringPackRequest: BuyAcquiringPackRequest; // As defined in SS-SPEC-001, Section 8.3.
}
```
...
| `PENDING` | The broadcast-service has received the submission and is performing full validation per SS-SPEC-001 Section 14. |
...
Failure categories correspond to those defined in SS-SPEC-001, Section 17.
...
**Person-to-person transfers.** The Stablecoin Stack architecture supports direct person-to-person token transfers in addition to merchant payment flows. A future wallet specification (SS-SPEC-009) will define the client-side interface for this use case. The gateway's `SUBMIT_PAYMENT` flow accommodates this without protocol changes; the beneficiary address need not be a registered merchant.

**Wallet certification.** SS-SPEC-009 (planned) will define a conformance test suite and certification programme for wallet client implementations, including tests of correct gateway interaction.
...
|---|---|---|---|
| 1.0.0 | 2026-03-10 | Fabric Payment Standards Foundation | Initial release. |
