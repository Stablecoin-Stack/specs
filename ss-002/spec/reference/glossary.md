...
Terms used in SS-SPEC-004 and the wallet-gateway documentation. Terms carried forward from SS-SPEC-001 are noted as such.

For the SS-SPEC-001 glossary, see the [SS-SPEC-001 Glossary](../../ss-001/reference/glossary.md).
...
| **Gateway Message** | A JSON object transmitted over the WebSocket connection, conforming to the message envelope defined in SS-SPEC-004, Section 5.2. |
...
| **Domain Separator** | A `bytes32` EIP-712 domain separator hex string. Used as the primary identifier for a supported token throughout the wallet-gateway interface. Full token metadata (including contract address, name, decimals, version, and network identifier) is available from the Basic Data Service. *(From SS-SPEC-001)* |
| **Transfer History** | The record of ERC-20 `Transfer` events for supported tokens associated with a given wallet address. Transfers of unsupported tokens and all other on-chain activity are excluded. *(From SS-SPEC-001)* |
...
| **Permit Nonce** | The per-owner counter maintained by an ERC-2612 token contract. Each successfully executed permit increments the nonce, invalidating all previously issued permits for that owner. The gateway provides nonce retrieval via `GET_NONCE` so wallets need not query the token contract directly. *(From SS-SPEC-001)* |
| **Payload ID** | A client-generated identifier (UUID) included in a `TransferRequest` submission. Used by the gateway and broadcast-service to enforce idempotency and correlate status notifications back to the originating submission. *(From SS-SPEC-001)* |
| **Zero-UUID** | The Acquirer ID consisting entirely of zero bytes (`0x00000000000000000000000000000000`). Passed as `acquirerId` in `GET_FEES` and in the payment payload when no acquirer is involved. *(From SS-SPEC-001)* |
...
- [SS-SPEC-001 Glossary](../../ss-001/reference/glossary.md)
